...
class Coin(pg.sprite.Sprite):
    def __init__(self):
        super().__init__()
        var = random.choices([0, 1, 2, 3], weights = [70, 20,9, 2])[0] #рандом выбор, с вероятность выпадения каждого
        if var == 0:
            self.image = pg.image.load("LAB8/files/coin.png") #загружаем разные фотки при разных значениях переменной
            self.cnt = 1 #стоимость монетки
        elif var == 1:
            self.image = pg.image.load("LAB8/files/coins.png")
            self.cnt = 5
        elif var == 2:
            self.image = pg.image.load("LAB8/files/manycoins.png")
            self.cnt = 10
        elif var == 3:
            self.image = pg.image.load("LAB8/files/anime.png")
            self.cnt = 100
    ...
    def move(self): 
        var = random.choices([0, 1, 2, 3], weights = [70, 20,9, 2])[0] #тоже самое, что и выше
        if var == 0:
            self.image = pg.image.load("LAB8/files/coin.png")
            self.cnt = 1
        elif var == 1:
            self.image = pg.image.load("LAB9/files/coins.png")
            self.cnt = 5
        elif var == 2:
            self.image = pg.image.load("LAB9/files/manycoins.png")
            self.cnt = 10
        elif var == 3:
            self.image = pg.image.load("LAB9/files/anime.png")
            self.cnt = 100
        self.image = pg.transform.scale(self.image, (30, 30)) #меняем размеры x - длина, y - ширина
        self.rect = self.image.get_rect()
...
while True:
    ...
    speed = money /10 + 1 #увеличивем скорость при сборе монеток
    ...
    speedtext = font_small.render("Speed: {}x".format(str(speed)), 1, black)
    screen.blit(scores, (10,10)) #отображаем текст
    ...
    if money >= 100: #функция выигрыша, если монет больше или ровно 100
        screen.fill(white)
        pg.mixer.Sound("LAB8/files/level.mp3").play()
        win_text = font.render("EZ WIN!!!!!!", 1, pg.Color("black"))
        screen.blit(win_text, (30, 250))
        screen.blit(scores, (60,350))
        screen.blit(money_text, (250, 350))
        pg.display.update()
        for x in all_sprites:
