...
if user_name == 'clear':
    ...
    quit()
if functions.check_name(user_name):
    mylist, tup, snakes = functions.get_data_user(user_name)
    score = mylist[0]
    lenght = mylist[1]
    level = mylist[2]
    fps = mylist[3]
    ...
    print('Успешно загружены сохранения!')
else:
    startgame = False
    functions.add_new_user(user_name)
    snake = [(x,y)]
    print('Новый игрок добавлен!')
def gameover(): #функция при проигрыше 
    ...
    pg.display.flip()
    functions.delete_data(user_name)
...
while True:
    ...
    if pressed[pg.K_ESCAPE]:
        functions.save_data_user(user_name, score, lenght, level, fps)
        for cords in walls_cord:
            functions.save_wall(user_name, cords[0], cords[1])
        for cords in snake:
            functions.save_snake(user_name, cords[0], cords[1])
    ...
    if startgame == True:
        hit_text = font_end.render("БЕССМЕРТИЕ", True, pg.Color("RED"))
        screen.blit(hit_text, (175, 350))
        if pg.time.get_ticks() - start_time > 10000:
            startgame = False
